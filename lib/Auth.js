    ...
    serveWebSocket(req, socket, next){
        ...
        if(Object.keys(req.body).length){
            switch(req.body.meta){
                case 'login':
                    this.loginWs(req, socket, next)
                break
                case 'logout':
                    this.logoutWs(req, socket, next)
                break
                case 'signin':
                    if(this.signinActive){
                        this.signinWs(req, socket, next)
                    }
                    else{
                        next(401)
                    }
                break
                default:
                    this.verifyWs(req, socket, next)
                break
            }
        }
        else{
            socket.send(JSON.stringify({ err : 'internal error', data : 500}))
        }
